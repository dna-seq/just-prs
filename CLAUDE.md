...
| `<cache>/reference/` | Runtime reference-allele universe pulled from HF (`reference_allele_universe.parquet`, via `PRSCatalog._reference_universe_path`) |
| `<cache>/percentiles/` | Distribution and quality parquets (`{panel}_distributions.parquet`, `{panel}_quality.parquet`, `{panel}_distribution_quality_issues.parquet`) plus the precompute output `reference_allele_universe.parquet` |
...
| `reference_allele_universe` | `compute` | Unions catalog scoring positions missing a `reference_allele` (`_ref_resolution_targets`), runs `resolve_reference_alleles()` (panel `.pvar` tier → FASTA SNV tier) → `percentiles/reference_allele_universe.parquet` `(genome_build, chrom, pos, ref, ref_source)`. Metadata logs `n_panel`/`n_fasta`/`n_unresolved`/`resolved_ratio` |
| `hf_reference_allele_universe` | `upload` | Pushes `reference_allele_universe.parquet` to `just-dna-seq/pgs-catalog` (`data/reference/`) via `push_reference_allele_universe()` |
