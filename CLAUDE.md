...
Key methods: `scores()`, `search()`, `best_performance()`, `publications()`, `score_info_row()`, `compute_prs()`, `compute_prs_batch()`, `percentile()`, `absolute_risk()`, `reference_data_status()`, `reference_distributions()`, `reference_individual_scores()`, `build_cleaned_parquets()`, `push_to_hf()`. The `absolute_risk(pgs_id, z_score, sex=None)` method joins scores, best_performance, prevalence, and publications data to produce an `AbsoluteRisk` estimate using the best available method (OR-per-SD or AUC-bivariate-normal). `score_info_row(pgs_id, genome_build=None)` adds a build-relative **`is_harmonized`** when a build is given (original dev build ≠ requested ⇒ scored via a harmonized file). `reference_individual_scores(pgs_id, superpopulation=None, panel="1000g")` returns the **per-individual** reference PRS `(iid, superpopulation, prs_score)` from the cached `reference_scores/{panel}/{pgs_id}/scores.parquet` (raw values for an empirical cohort histogram, vs the aggregated `reference_distributions()`); no recompute/download — raises `FileNotFoundError` when not locally cached (per-individual scores are pipeline output, not published to HF).
