...
| `just_prs.prs` | `compute_prs()` / `compute_prs_duckdb()` / `compute_prs_batch()` — core PRS engines. Two engines: **polars** (lazy in-memory, default for API) and **DuckDB** (SQL, spills to disk, default in UI). `PRSEngine` str enum (`POLARS`, `DUCKDB`) for type-safe engine selection and `GenotypeInputMode` str enum (`AUTO`, `VARIANT_ONLY`, `ALL_SITES`, `PLINK_PRESENT_ONLY`) for absent-locus semantics. `compute_prs_duckdb()` accepts `genotypes_parquet` (preferred — DuckDB reads directly) or `genotypes_lf`, plus `memory_limit` param (falls back to `PRS_DUCKDB_MEMORY_LIMIT` env var, then 75% of RAM). Both engines support standard additive (`effect_weight`) and per-dosage (GenoBoost) weight formats, theoretical stats, and percentile computation. `is_dosage_weight_format()` detects the format; `_normalize_scoring_columns()` handles both transparently. Both engines accept `resolve_reference=False` + `reference_universe_path=` (variant-only only): when on, `_apply_reference_resolution()` fills a missing `reference_allele` from the precomputed `reference_allele_universe.parquet` before the dosage chain, dissolving `variants_unscorable_absent` and recording `variants_ref_resolved_panel` / `variants_ref_resolved_fasta` (subsets of `variants_assumed_hom_ref`) on `PRSResult`. `PRSCatalog.compute_prs(...)` / `compute_prs_batch(...)` lazily pull the universe from HF on cache miss (same pattern as `quality_label`). **Default is OFF and the caller opts in** — it is correct only for genome-wide variant-only WGS (where absent ≈ hom-ref), validated there (coverage ~27%→~99.9%, `variants_unscorable_absent` ~7.5M→~6.6K). It must stay off for unknown inputs and is NOT correct for arrays: a normalized array auto-infers `VARIANT_ONLY` (no `NON_REF`/`RefCall` markers), so enabling it would impute phantom hom-ref at untyped off-chip positions. Arrays need a chip-aware path instead (hom-ref only at chip-typed positions = universe ∩ `chip_coverage` typed set; see refcall plan item D). The flag degrades to a no-op when the universe parquet is unavailable. CLI: `prs compute --resolve-reference` (default off). Rebuild/publish the universe with `scripts/build_reference_allele_universe.py [--push]`. |
