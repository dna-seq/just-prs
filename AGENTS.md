...
| `just_prs.liftover` | **GRCh37 ⇄ GRCh38 coordinate liftover** — canonical wrapper around pure-Python `pyliftover` + UCSC chain files (do not import `pyliftover` directly). `normalize_build()` (reuses `cleanup.BUILD_NORMALIZATION`; 37/38 only), `LiftOver.lift_position()` (strand-`+` only) / `lift_position_full()` / `lift_records()` → `LiftRecordResult(lifted, dropped)` with `liftover_reason ∈ {missing_coordinates, invalid_position, unmapped, strand_flipped}`, `get_liftover()` (lru-cached), `liftover_preflight()`, `download_chain_file()`, `lift_frame(df, src, tgt)` → `(lifted, dropped)` (vectorized polars lift; dropped rows carry `liftover_reason`). 1-based VCF coords in/out; `chr`-style round-tripped. Chains auto-download to `<cache>/liftover/`. **Chip-agnostic** (chip-extension contract). Optional `[liftover]` extra (`pyliftover>=0.4`, pure-Python → installs on Windows, no marker); lazy-imported via `_require_pyliftover()`. Design borrowed from genomi's `runtime/liftover.py`. Wired into the array lift bridge (`array_scoring.compute_array_prs(target_build=...)`). |
...
| `just_prs.ancestry` | **Sample genetic-ancestry inference (Level 1).** Vendored FRAPOSA OADP projection (`_projection.py`, MIT — vendored because `fraposa-pgsc` pins `numpy<2`, license is clean) + a pure-numpy KNN classifier. `build_ancestry_model(...)` does numpy SVD + leave-one-out validation and persists the artifact (sites+loadings parquet, reference-PC parquet, meta JSON); `infer_ancestry(model_dir, genotypes_lf, *, panel, build)` → `AncestryInference` (super-pop label, probabilities, PC coords, coverage; `UNKNOWN` below a coverage floor). Allele-aware site matching reuses `vcf.compute_dosage_expr`. Runtime is **pure-Python — no plink2/GPL/binary, Windows-clean**; the model is built offline by the Dagster `ancestry_model_pipeline` (plink2 LD-prune + SVD, **GRCh38 only**; GRCh37 samples lift to 38) and pulled from HF (`hf.pull_ancestry_model`, catalog repo `data/ancestry/`, alongside the universes). **Level 2 `estimate_proportions`** → ancestry *fractions* (`mixture`, pure-numpy simplex LSQ of the PC vector onto super-pop centroids). **`bayesian_consensus`** fuses each panel's KNN posterior + mixture (canonicalized via `to_canonical_superpops`: HGDP `CSA→SAS`, `MID` dropped) into one posterior (Laplace-smoothed product-of-experts). See [docs/sample-ancestry-methodology.md](docs/sample-ancestry-methodology.md). |
...
| `just_prs.array_scoring` | `compute_array_prs(..., genome_build="GRCh37", target_build=None, ...)`: normalize → detect chip → optional **liftover** → LD-proxy → MAF fill → score → coverage tier. **Lift bridge (`target_build` set):** lifts genotypes `genome_build→target_build` (`liftover.lift_frame`), scores against target-build harmonized files + universe (→ target-build percentiles). Restoration scope in the lift path = chip typed positions **lifted into the target build** (`_lifted_chip_positions`, cached); a position that fails to lift is excluded from scope, so lift-dropped loci are never assumed hom-ref. Native path (`target_build` None) uses `_resolve_array_restoration` unchanged. `ArrayPRSResult` adds `source_build`/`lifted_to_build`/`genotypes_lift_dropped`. |
...
Key methods: `scores()`, `search()`, `best_performance()`, `publications()`, `development_ancestry()`, `score_info_row()`, `compute_prs()`, `compute_prs_batch()`, `percentile()`, `absolute_risk()`, `reference_data_status()`, `reference_distributions()`, `reference_individual_scores()`, `build_cleaned_parquets()`, `push_to_hf()`. The `absolute_risk(pgs_id, z_score, sex=None)` method joins scores, best_performance, prevalence, and publications data to produce an `AbsoluteRisk` estimate using the best available method (OR-per-SD or AUC-bivariate-normal). `score_info_row(pgs_id, genome_build=None)` adds build-relative **`is_harmonized`** when a build is given. `reference_individual_scores(pgs_id, superpopulation=None, panel="1000g")` returns per-individual reference PRS `(iid, superpopulation, prs_score)` from the cached `reference_scores/{panel}/{pgs_id}/scores.parquet` (raw values for an empirical cohort histogram; raises `FileNotFoundError` if not locally cached — not published to HF). `development_ancestry(pgs_id=None)` returns the per-score **development**-ancestry row(s) from `clean_score_development_samples()` (F19/F21); `scores()` left-joins the lean scalars (`dev_ancestry_broad`, `dev_sample_size`, `dev_is_multi_ancestry`) into the wide grid while `score_info_row()` merges the full breakdown (incl. `dev_ancestry_distribution`, per-stage `gwas_ancestry_broad`/`training_ancestry_broad`). Old caches missing the parquet are transparently rebuilt from the raw FTP sheet (same pattern as `publications.parquet`). `infer_ancestry(genotypes_path|genotypes_lf, *, panel="1000g", sample_build=None)` infers the **sample's** super-population via `just_prs.ancestry` (implicit build GRCh38 when undetected, lifts if sample build ≠ model build, lazily pulls the model from HF) → `AncestryInference`. `assess_ancestry_coherence(pgs_id, ancestry, *, genome_build=None)` → `AncestryCoherence` verdict (coherent / panel_mismatch / dev_mismatch / both / unknown) comparing sample vs score-development (`development_ancestry`) vs percentile-panel (`reference_data_status`) ancestry, with plain-English text; uses the curated `_PGS_BROAD_TO_SUPERPOP` map (admixed labels abstain). `infer_ancestry_consensus(genotypes, panels=("1000g","hgdp_1kg"), include_prive=False)` fuses every panel's KNN + mixture (+ Privé) into an `AncestryConsensus` (`bayesian_consensus`). `infer_ancestry_prive(genotypes)` → Privé/bigsnpr 21-group proportions + continental rollup (`just_prs.ancestry.prive`: UKB ref, GRCh37, 16-PC projection + shrinkage correction + simplex-QP; built locally, GPL data not HF-published). CLI: `prs ancestry infer --mode label|mixture|prive|consensus|all [--prive] [--resolution superpop|population]` (default `all`). `resolution="population"` classifies at the model's fine-pop level (HGDP 73 pops incl. East-Slavic `Russian`, LOO 25/25); broad rollup kept in `superpopulation`, fine call in `fine_population`. Bounded by PC depth + biology (West-Slavic≈Germanic); full Slavic granularity via the **`aadr_ho` panel** (`just_prs.ancestry.{eigenstrat,aadr}`: builds from local AADR Human Origins present-day West-Eurasians, GRCh37, 194 pops; `PRSCatalog._ANCESTRY_PANEL_BUILD` lifts samples 38→37; built locally, not HF-published). Soft proportions resolve East-Slavic/Balkan (anton→40% Ukrainian, livia→Moldavian/Balkan); hard sub-label limited by biology (East Slavs ≈ one autosomal cluster → use the distribution, not the hard call). **Result metafield:** `PRSResult.sample_ancestry` (compact `SampleAncestry`: consensus super-pop+confidence, fine population+confidence+panel+soft mixture, informational super-pop mixture) is filled by `compute_prs`/`compute_prs_batch(infer_ancestry=True, ancestry_include_prive=, ancestry_include_aadr=)` — inferred once per sample and shared across batch results via `PRSCatalog.infer_sample_ancestry()` (super-pop from the superpop-resolution consensus, fine call from a separate population-resolution pass on the finest in-play panel). CLI: `prs compute --ancestry [--ancestry-prive] [--ancestry-aadr]`.
