...
`pyliftover` is **optional** too, in a separate `liftover` extra (`pip install just-prs[liftover]`), needed only for GRCh37⇄GRCh38 coordinate liftover (`just_prs.liftover`). Unlike `pgenlib`/`pysam` it is **pure-Python with no `sys_platform` marker**, so it installs everywhere including Windows; it is lazy-imported and raises a clear `ImportError` when missing. The workspace root depends on `just-prs[reference,liftover]`.
...
| `just_prs.cleanup` | Pure-function pipeline: genome build normalization, column renaming, metric string parsing, performance metric cleanup, publications cleanup, development-ancestry aggregation. `clean_score_development_samples()` collapses the `score_development_samples` sheet (up to ~75 rows/PGS across the GWAS-discovery and Score-Development/Training stages) into one row per `pgs_id` with the **development** ancestry (distinct from the *evaluation* ancestry): `dev_ancestry_broad` (dominant, sample-weighted), `dev_ancestries`, `dev_ancestry_distribution` (JSON `{ancestry: fraction}`), `dev_is_multi_ancestry`, `dev_sample_size` (larger of GWAS/training stage totals), and per-stage breakdowns. This is the F19/F21 development-ancestry surface (the structured input a score×sample×panel coherence veto would read; the veto itself stays deferred P3). |
...
| `just_prs.prs` | `compute_prs()` / `compute_prs_duckdb()` / `compute_prs_batch()` — core PRS engines. Two engines: **polars** (lazy in-memory, default for API) and **DuckDB** (SQL, spills to disk, default in UI). `PRSEngine` str enum (`POLARS`, `DUCKDB`) for type-safe engine selection and `GenotypeInputMode` str enum (`AUTO`, `VARIANT_ONLY`, `ALL_SITES`, `PLINK_PRESENT_ONLY`) for absent-locus semantics. `compute_prs_duckdb()` accepts `genotypes_parquet` (preferred — DuckDB reads directly) or `genotypes_lf`, plus `memory_limit` param (falls back to `PRS_DUCKDB_MEMORY_LIMIT` env var, then 75% of RAM). Both engines support standard additive (`effect_weight`) and per-dosage (GenoBoost) weight formats, theoretical stats, and percentile computation. `is_dosage_weight_format()` detects the format; `_normalize_scoring_columns()` handles both transparently. Both engines accept **`reference_restoration: RestorationScope`** (`bool \| Chip \| Path \| pl.DataFrame`, default `False`) + `reference_universe_path=` to restore a missing `reference_allele` from the precomputed `reference_allele_universe.parquet` so absent loci score as hom-ref — **scope-gated** (variant-only only): `False` = off; `True` = whole universe (WGS — the universe index *is* the eligible set, no duplication); `Chip` = chip-typed positions ∩ universe (arrays); `Path`/`DataFrame` = a custom `(chrom,pos)` set (embedder escape hatch). `_normalize_restoration_scope()` resolves the scope (build-gated for `Chip` — no-op + log when no build-matched manifest **or** no published universe for that build; GSA now ships both A2/GRCh38 and A1/GRCh37 manifests, so `Chip` scope unlocks per build as soon as that build's universe is published); `_apply_reference_resolution()` fills only null `reference_allele` from the eligible REF set and records `variants_ref_resolved_panel`/`_fasta` (subsets of `variants_assumed_hom_ref`); the join is **flipped to hash the small scoring side** (reduce the in-memory universe to scoring positions via a semi-join, then left-join back) so it never hashes the 34M-row universe (~6× faster, results identical to the old `scoring.join(universe)`). **Prepare-once / dependency injection (perf, F-restore-once):** the universe is catalog-wide and identical for every score, so re-parsing + re-joining it per score dominated multi-score runs (~8.4→~1.2 s/score). **`prepare_reference_universe(source, *, genome_build, scope=None) -> ReferenceUniverse`** parses (or accepts an injected `DataFrame`/`LazyFrame`/`Path`) and optionally pre-subsets **once**; pass the handle into `compute_prs`/`compute_prs_duckdb`/`compute_prs_batch` (+ `PRSCatalog.*`) via **`reference_universe=`** — it takes precedence over `reference_universe_path`, skips the re-parse, and the scope is baked in (the handle's presence **is** the restoration intent, so an embedder can fully own universe loading/subsetting). `compute_prs_batch` (module + `PRSCatalog`) builds the handle **once before the loop** when restoration is on and none was injected, so even the non-injecting batch pays the universe cost once; the corrupt-cache retry path now threads restoration too. `PRSCatalog.prepare_reference_universe(genome_build, *, reference_restoration=)` HF-pulls + parses the build-aware universe into a handle. Restoration stays variant-only-gated; in DuckDB the genotype mode is resolved up front (cheap schema probe) so restoration is skipped outside variant-only. The universe filename is **build-aware** via `hf.reference_allele_universe_filename(build)` (GRCh38 keeps the historical unsuffixed `reference_allele_universe.parquet` for back-compat; other builds are `_<build>`-suffixed, e.g. `reference_allele_universe_GRCh37.parquet`); `PRSCatalog._reference_universe_path(genome_build)`, `hf.push_/pull_reference_allele_universe(..., genome_build=)`, and `array_scoring._resolve_array_restoration` all resolve it per build. **Default OFF, caller opts in** — `True` is correct only for genome-wide variant-only WGS (validated: coverage ~27%→~99.9%, `variants_unscorable_absent` ~7.5M→~6.6K); unknown → off; arrays → `Chip` (a blanket `True` on an array would phantom-hom-ref untyped off-chip positions). Degrades to no-op when the universe is unavailable. `PRSCatalog.compute_prs/compute_prs_batch` thread the scope + lazily pull the universe; CLI `prs compute --reference-restoration [off\|wgs\|<chip>]` (default off). Rebuild/publish via `scripts/build_reference_allele_universe.py [--push] [--genome-build GRCh38\|GRCh37]` (the build script sets `PRS_PIPELINE_GENOME_BUILD`, which drives the build-parameterized universe assets). **Full flow + the gz edge case: [docs/vcf_flow.md](docs/vcf_flow.md).** GRCh37 universe build: [docs/grch37-universe-build.md](docs/grch37-universe-build.md). **Build-mismatch guard:** `compute_prs`/`compute_prs_duckdb`/`compute_prs_batch` (+ `PRSCatalog`) accept **`sample_build=`** (via `_assert_sample_build_matches`); when set and it differs from `genome_build` (the build the scoring file is in) they **raise** a clear error instead of silently matching ~0 variants across builds. `prs compute` auto-detects it via `detect_genome_build(vcf)` (None ⇒ no guard, never guesses); the fix for a real mismatch is to lift (`compute_array_prs(target_build=)` / `liftover.lift_frame`). |
| `just_prs.reference` | Reference panel utilities and pgen operations: `download_reference_panel()` (panel-aware: `panel="1000g"` or `"hgdp_1kg"`), `parse_pvar()` (parse .pvar.zst with parquet caching), `parse_psam()` (parse .psam sample files), `read_pgen_genotypes()` (extract genotypes from .pgen via pgenlib), `match_scoring_to_pvar()` (allele-aware variant matching via polars — standalone use only), `compute_reference_prs_polars()` (single-PGS scoring using pgenlib + numpy; uses DuckDB for variant matching against pvar parquet to avoid loading 75M rows into memory), `compute_reference_prs_batch()` (memory-efficient batch scoring: resolves panel once via `_ResolvedRefPanel`, uses DuckDB for variant matching, aggregates distributions per PGS ID immediately and discards raw scores, returns `BatchScoringResult`), `compute_reference_prs_plink2()` (legacy, for cross-validation), `aggregate_distributions()`, `distribution_quality_issues()` (one row per non-finite/zero-variance distribution anomaly for manual triage and exclusion decisions), `enrich_distributions()` (join distributions with cleaned metadata: traits, EFO, AUROC, OR, C-index, ancestry), `ancestry_percentile()`, `ReferencePanelError`. Panel-aware constants: `REFERENCE_PANELS` dict, `DEFAULT_PANEL = "1000g"`. Result models: `ScoringOutcome` (per-ID outcome), `BatchScoringResult` (panel, distributions_df, outcomes, quality_df, distribution_issues_df — no raw scores held in memory). `_ResolvedRefPanel` caches file paths, psam, and variant count once per batch; variant matching uses DuckDB to scan the pvar parquet (~434 MB on disk) without materializing 75M rows in polars (~6 GB). Also hosts the **reference-allele FASTA resource** (precompute-only): `REFERENCE_FASTA` registry (Ensembl primary assembly per build, release via `PRS_ENSEMBL_RELEASE`), `download_reference_fasta()` (streams the `.fa.gz`, decompresses to a plain `.fa` since faidx cannot index plain-gzip, builds `.fai` via `pysam.faidx`), `reference_fasta_path()`, and `REFERENCE_FASTA_CHR1_LENGTH` (chr1-length build fingerprint for the contig/build guard). |
| `just_prs.liftover` | **GRCh37 ⇄ GRCh38 coordinate liftover** — the canonical wrapper around pure-Python `pyliftover` + UCSC chain files (callers must not import `pyliftover` directly). `normalize_build()` (reuses `cleanup.BUILD_NORMALIZATION` + short aliases; 37/38 only), `LiftOver(src,tgt).lift_position()` (strand-`+` only) / `lift_position_full()` → `(chrom,pos,strand)` / `lift_records()` → `LiftRecordResult(lifted, dropped)` tagging each drop with `liftover_reason ∈ {missing_coordinates, invalid_position, unmapped, strand_flipped}`, `get_liftover()` (lru-cached), `liftover_preflight()` (readiness without download), `download_chain_file()`, and `lift_frame(df, src, tgt, *, chrom_col, pos_col)` → `(lifted_df, dropped_df)` (vectorized polars lift: distinct positions lifted once + joined back; preserves columns/row order; dropped rows carry `liftover_reason`). **1-based VCF coords in/out** (converts to pyliftover's 0-based BED at the boundary); `chr`-prefix style is round-tripped to match the caller. Chain files (`hg19ToHg38`/`hg38ToHg19.over.chain.gz`, ~3 MB) auto-download from UCSC goldenPath to `<cache>/liftover/`. **Deliberately chip-agnostic** — a genotyping chip never gets special lift logic (chip-extension contract). Optional `[liftover]` extra (`pyliftover>=0.4`); pure-Python so it installs on Windows (no `sys_platform` marker), lazy-imported via `_require_pyliftover()`. Design borrowed from the genomi project's `runtime/liftover.py`. Consumed by the **array lift bridge** in `array_scoring.compute_array_prs(target_build=...)`: GRCh37 array → lift genotypes 37→38 → score against GRCh38 harmonized files + universe → GRCh38 percentile. |
| `just_prs.reference_allele` | **Reference-allele resolution** for absent PGS scoring variants (the F15 coverage lever). `resolve_reference_alleles(positions_df, genome_build, *, panel_pvar_path=, fasta_path=)` resolves the REF base at a set of `(chrom,pos)` positions in two tiers: **panel** (reference-panel `.pvar` REF via DuckDB scan — free, indel-correct) then **FASTA** (single-base SNV faidx via `pysam` for the long tail). Returns `(genome_build, chrom, pos, ref, ref_source ∈ {panel, fasta, unresolved})`. FASTA tier resolves single-base SNV only (multi-base/indel positions stay `unresolved`); a contig/build mismatch (chr1-length fingerprint) **fails loudly** rather than injecting a wrong base. Self-contained / extractable into a future `just-vcf-ops`. `ref_source` is a `RefSource` StrEnum. Built once into `reference_allele_universe.parquet` by the pipeline and consumed at runtime by `prs.py` via `reference_restoration` (see the `just_prs.prs` row). |
| `just_prs.io_utils` | `open_maybe_compressed(path)` / `is_gzip(path)` — open a file decompressing gzip/BGZF detected by **magic bytes** (`1f 8b`), not extension. Used by `detect_genome_build` and `arrays._read_array_text` so a BGZF stream named `.vcf` (or a gzipped array without `.gz`) is read correctly. Fixes the historical extension-based gz bug (see [docs/vcf_flow.md](docs/vcf_flow.md)). |
| `just_prs.ancestry` | **Sample genetic-ancestry inference (Level 1).** Vendored FRAPOSA OADP projection (`_projection.py`, MIT — `standardize`/`svd_online`/`procrustes`/`oadp`/`build_reference_pca`/`project_samples`; vendored because `fraposa-pgsc` pins `numpy<2`, license is clean) + a pure-numpy KNN classifier. `build_ancestry_model(genotypes, sites, labels, panel, build, model_dir)` does the numpy SVD + leave-one-out validation and persists the artifact (sites+loadings parquet, reference-PC parquet, meta JSON). `infer_ancestry(model_dir, genotypes_lf, *, panel, build)` → `AncestryInference` (super-pop label, classifier probabilities, PC coords, coverage; `UNKNOWN` below a coverage floor). Allele-aware site matching reuses `vcf.compute_dosage_expr`. Runtime is **pure-Python — no plink2/GPL/binary, Windows-clean**; the model is built offline by the Dagster `ancestry_model_pipeline` (plink2 LD-prune + SVD, **GRCh38 only**; GRCh37 samples lift to 38) and pulled from HF (`hf.pull_ancestry_model`, catalog repo `data/ancestry/`, alongside the universes). **Level 2 — `estimate_proportions`** returns ancestry *fractions* (`mixture`, `mixture_method="pca_nnls"`) via a pure-numpy simplex-constrained least-squares of the PC vector onto the super-pop centroids. **Consensus — `bayesian_consensus`** fuses several method distributions (each panel's KNN posterior + PCA-NNLS mixture, canonicalized to the 1000G 5 super-pops via `to_canonical_superpops`: HGDP `CSA→SAS`, `MID` dropped) into one posterior via a Laplace-smoothed product-of-experts. See [docs/sample-ancestry-methodology.md](docs/sample-ancestry-methodology.md). |
| `just_prs.chip_coverage` | Consumer-genotyping-chip coverage of PGS scoring files. `CHIPS` / `CHIPS_BY_ID` define supported chips (currently `gsa_v3` — Illumina Global Screening Array v3, the platform the current consumer market converged on: 23andMe v5, AncestryDNA v2, MyHeritage 2019+, FamilyTreeDNA v2, LivingDNA). `download_chip_manifest(chip, cache_dir, *, build=)` fetches the GSA per-build manifest zip from the chip's `manifests` map (**A2** = GRCh38, **A1** = GRCh37); `parse_gsa_manifest()` extracts ~648K typed `(chr_norm, pos)` markers (format-identical across A1/A2); `chip_typed_positions(chip, cache_dir, *, build=)` caches unique positions to parquet per build (raises `ValueError` only for a build the chip has no manifest for — no longer `NotImplementedError` for GRCh37); `compute_chip_coverage()` intersects each GRCh38 scoring parquet's `hm_chr`/`hm_pos` against the chip's typed positions and returns one row per `pgs_id × chip` with `n_typed`, `n_total`, `coverage_ratio`, and `array_ready` (bool: `coverage_ratio >= ARRAY_READY_THRESHOLD`, default 0.90, and the score has mapped coordinates). Answers "which PRS are array-ready vs imputation-required" per chip. **Coverage is a position-set intersection in a single build** — A2=GRCh38 means no liftover is needed against the GRCh38 harmonized scoring files. The GSA manifest is the shared platform core; it omits each vendor's custom add-on markers, so coverage is a slight under-estimate. Older arrays (Illumina OmniExpress — pre-2019 kits — and deCODEme Omni) are a different platform and not represented. **Imputation itself is per-individual and cannot be precomputed/downloaded** (it infers untyped genotypes from one person's observed alleles against a reference panel); only this static per-chip coverage map is precomputable. `PRSCatalog.scores()` left-joins the coverage (pivoted to wide per-chip columns `{chip}_array_ready` / `{chip}_coverage`) via `_load_chip_coverage()`, pulling `chip_coverage.parquet` from HF (`pull_chip_coverage`) on local miss — same lazy pattern as `quality_label`. |
| `just_prs.arrays` | Consumer genotyping-array ingestion. `normalize_array()` parses a 23andMe / AncestryDNA / MyHeritage raw file (`.txt`/`.txt.gz`/`.csv`/`.zip`) into the **same** normalized Parquet schema as `normalize_vcf()` (`chrom`, `pos`, `rsid`, `ref`, `alt`, `GT`, `genotype`), so `compute_prs()` / `compute_prs_duckdb()` consume array data unchanged. `detect_array_format()` auto-detects vendor: 4-col TSV 23andMe, 5-col TSV AncestryDNA, or comma-CSV `myheritage` (header `RSID,CHROMOSOME,POSITION,RESULT` — also covers FamilyTreeDNA's identical layout, by vendor name or comma-delimiter shape). **Encoding trick:** arrays report observed alleles directly, so het `a1≠a2` → `ref=a1,alt=a2,GT="0/1"` and hom `a1==a2` → `ref=alt=a,GT="1/1"`, which makes the existing GT/ref/alt dosage logic count the effect allele correctly. Defaults to `genome_build="GRCh37"` (the build 23andMe v5 / AncestryDNA v2 report) — the caller must score against the matching GRCh37 harmonized file. **Known limitations** (documented, not silently handled): strand flips, indels (`I`/`D` codes), and hemizygous male X/Y (treated as homozygous). |
| `just_prs.array_scoring` | High-level array PRS: `compute_array_prs(array_path, scoring_file, genome_build="GRCh37", target_build=None, ...)` chains normalize → detect chip → (optional **liftover**) → LD-proxy → MAF fill → score → coverage tier (`ArrayPRSResult`). **Lift bridge (`target_build` set, e.g. `"GRCh38"`):** the array is normalized in its native build, its genotypes are lifted `genome_build→target_build` via `liftover.lift_frame`, and it is scored against the **target-build** harmonized files + universe so the result rides that build's reference distributions/percentiles. Restoration in the lift path uses `_lifted_chip_restoration`: the scope is the chip's typed positions **lifted into the target build** (`_lifted_chip_positions`, cached per chip+src→tgt) — a typed position that fails to lift is deliberately **excluded** from the scope, so a sample locus dropped by the same lift can never be assumed hom-ref in the target build (the "pad-first / drop-after" correctness, realized via the existing custom-`(chrom,pos)`-DataFrame restoration scope, no genotype materialization). Native (no-`target_build`) path is unchanged and uses `_resolve_array_restoration` (Chip scope ∩ same-build universe). `ArrayPRSResult` gains `source_build` / `lifted_to_build` / `genotypes_lift_dropped`. |
...
| `just_prs.vcf` | VCF reading via `polars-bio`, genome build detection, dosage computation. `scan_vcf()` uses a content-aware named DataFusion registration because upstream `polars_bio.scan_vcf()` derives a table name from the file stem and can panic when the same VCF is read concurrently. Route package VCF scans through this helper rather than calling `polars_bio.scan_vcf()` directly. |
...
| `just_prs.quality` | **The single home for ALL PRS quality classification — never reinvent quality thresholds anywhere else (not in `viz.py`, not in a mixin, not in the UI).** Pure-logic helpers: `classify_model_quality()` (genotype-aware: match_rate + AUROC → High/Moderate/Low/Very Low), `classify_synthetic_quality()` / `classify_combined_quality()` / `synthetic_quality_score()` / `synthetic_quality_tier()` (metadata-based 0–100 scoring + bands), `resolve_quality_key()` (canonical label→`high`/`moderate`/`low`/`very_low` tier key shared by the chart **and** the report table so they can never disagree — prefer the `quality_label` already on a scored row, fall back to variant-count/AUROC only for unscored reference-context models), `interpret_prs_result()`, `format_effect_size()`, `format_classification()`. No Reflex dependency -- shared between core library and UI. **Lesson:** the chart once classified quality by a private variant-count heuristic in `viz.py` while the table read a never-populated `quality` key (defaulted everything to "low"), so the graph showed High/Moderate but the table showed all Low. Consumers must read the `quality_label` carried on each result row (computed once in `enrich.py`) and normalize via `resolve_quality_key()` — do not classify quality inline. |
...
- **Column renaming**: Verbose PGS column names (e.g. `Polygenic Score (PGS) ID`) become snake_case (`pgs_id`). The full mapping is `_SCORES_COLUMN_RENAME` / `_PERF_COLUMN_RENAME` / `_EVAL_COLUMN_RENAME` / `_PUBLICATIONS_COLUMN_RENAME` / `_DEV_SAMPLES_COLUMN_RENAME`.
...
- **Development-ancestry aggregation** (F19/F21): `clean_score_development_samples()` turns the raw `pgs_all_metadata_score_development_samples.csv` sheet into one row per `pgs_id`. Per-stage rows (GWAS discovery vs Score-Development/Training) and per-ancestry partitions are collapsed into a sample-size-weighted broad-ancestry distribution. Buckets keep admixed labels intact (e.g. `"European, East Asian"`) for the distribution; `dev_ancestries` additionally splits them into the distinct set. This is the **development** (training) ancestry — distinct from the *evaluation* ancestry that `clean_performance_metrics()` exposes — and is the axis a coherence check needs.

### PRSCatalog class (`just_prs.prs_catalog`)

`PRSCatalog` is the primary interface for working with PGS Catalog data. It produces and persists 5 cleaned parquet files (`scores.parquet`, `performance.parquet`, `best_performance.parquet`, `publications.parquet`, `score_development_ancestry.parquet`) and loads them as LazyFrames. Loading uses a 3-tier fallback chain: local cleaned parquets -> HuggingFace pull -> raw FTP download + cleanup. Raw FTP parquets are cached separately in a `raw/` subdirectory to avoid collision with cleaned files.

Key methods: `scores()`, `search()`, `best_performance()`, `publications()`, `development_ancestry()`, `score_info_row()`, `compute_prs()`, `compute_prs_batch()`, `percentile()`, `absolute_risk()`, `reference_data_status()`, `reference_distributions()`, `reference_individual_scores()`, `build_cleaned_parquets()`, `push_to_hf()`. The `absolute_risk(pgs_id, z_score, sex=None)` method joins scores, best_performance, prevalence, and publications data to produce an `AbsoluteRisk` estimate using the best available method (OR-per-SD or AUC-bivariate-normal). `score_info_row(pgs_id, genome_build=None)` adds a build-relative **`is_harmonized`** when a build is given (original dev build ≠ requested ⇒ scored via a harmonized file). `reference_individual_scores(pgs_id, superpopulation=None, panel="1000g")` returns the **per-individual** reference PRS `(iid, superpopulation, prs_score)` from the cached `reference_scores/{panel}/{pgs_id}/scores.parquet` (raw values for an empirical cohort histogram, vs the aggregated `reference_distributions()`); no recompute/download — raises `FileNotFoundError` when not locally cached (per-individual scores are pipeline output, not published to HF). `development_ancestry(pgs_id=None)` returns the per-score **development**-ancestry row(s) from `clean_score_development_samples()` (F19/F21); `scores()` left-joins the lean scalars (`dev_ancestry_broad`, `dev_sample_size`, `dev_is_multi_ancestry`) into the wide grid while `score_info_row()` merges the full breakdown (incl. `dev_ancestry_distribution`, per-stage `gwas_ancestry_broad`/`training_ancestry_broad`). Old caches missing the parquet are transparently rebuilt from the raw FTP sheet (same pattern as `publications.parquet`). `infer_ancestry(genotypes_path|genotypes_lf, *, panel="1000g", sample_build=None)` infers the **sample's** super-population via the `just_prs.ancestry` engine — implicit build **GRCh38** when undetected, lifts via `liftover` if the sample build ≠ model build, lazily pulls the model from HF (`pull_ancestry_model`); returns `AncestryInference`. `assess_ancestry_coherence(pgs_id, ancestry, *, genome_build=None)` returns an `AncestryCoherence` verdict (coherent / panel_mismatch / dev_mismatch / both / unknown) comparing the sample super-pop against the score's development ancestry (`development_ancestry`) and the percentile reference panel (`reference_data_status`), with plain-English text; uses the curated `_PGS_BROAD_TO_SUPERPOP` map (admixed labels abstain). `infer_ancestry_consensus(genotypes, panels=("1000g","hgdp_1kg"))` runs single-panel inference on each panel (sample read+lifted to GRCh38 once, reused) and fuses every method (each panel's KNN posterior + PCA-NNLS mixture) into an `AncestryConsensus` via `bayesian_consensus`. `infer_ancestry_prive(genotypes)` returns the **Privé/bigsnpr 21-group** proportions (`just_prs.ancestry.prive`: UKB worldwide reference, GRCh37 — sample lifted 38→37, projected onto 16 PCs with the published shrinkage `correction`, simplex-QP against the group PC-centroids) + a continental rollup; built locally via `prive.build_prive_reference` (GPL data, not HF-published), folded into the consensus with `include_prive=True`. The CLI exposes all of it: `prs ancestry infer --mode label|mixture|prive|consensus|all [--prive]` (default `all`). **`resolution="population"`** (CLI `--resolution population`) classifies at the model's fine-population level instead of super-pop — e.g. `--panel hgdp_1kg --resolution population` surfaces HGDP's 73 pops incl. East-Slavic `Russian` (LOO 25/25); the broad rollup stays in `superpopulation`, the fine call in `fine_population`. Fine within-continent resolution is bounded by PC depth + biology (West-Slavic ≈ Germanic on top PCs); HGDP has only `Russian` among Slavs. The **`aadr_ho` panel** (`just_prs.ancestry.eigenstrat` reads packed-`TGENO`; `just_prs.ancestry.aadr.build_aadr_ho_model` builds from local AADR Human Origins present-day West-Eurasians — GRCh37, 194 pops incl. Russian/Ukrainian/Belarusian/Czech/Bulgarian/Romanian; `PRSCatalog` routes it to GRCh37 via `_ANCESTRY_PANEL_BUILD`; built locally, not HF-published) adds Slavic/Balkan resolution: the **soft proportions** place East-Slavic/Balkan correctly (anton→40% Ukrainian, livia→Moldavian/Balkan), but the **hard sub-label is limited by biology** (East Slavs are ~one autosomal cluster → Ukrainian/Belarusian collapse to Russian; use the distribution, not the hard call). **Result metafield:** `PRSResult.sample_ancestry` (a compact `SampleAncestry`: consensus `superpopulation`+`confidence`, finest `fine_population`+`fine_confidence`+`fine_panel`+`fine_mixture`, and the fused super-pop `mixture` as informational) is populated when `compute_prs`/`compute_prs_batch(infer_ancestry=True, ancestry_panels=, ancestry_include_prive=, ancestry_include_aadr=)` is requested — ancestry is a sample-level property so it's inferred **once** (VCF read once) and shared across all batch results. `PRSCatalog.infer_sample_ancestry(...)` builds it: super-pop from the canonical-5 consensus at `resolution="superpop"` (never poisoned by fine labels), fine call from a **separate** population-resolution inference on the finest in-play panel (`_FINE_PANEL_PRIORITY`: aadr_ho > hgdp_1kg > 1000g). CLI: `prs compute --ancestry [--ancestry-prive] [--ancestry-aadr]`.

The package public API (`just_prs.__init__`) exports: `PRSCatalog`, `ReferencePanelError`, `AbsoluteRisk`, `normalize_vcf`, `VcfFilterConfig`, `resolve_cache_dir`, `classify_model_quality`, `interpret_prs_result`, `format_effect_size`, `format_classification`, `PRSEngine`, `GenotypeInputMode`, `compute_reference_prs_polars`, `compute_reference_prs_batch`, `download_reference_panel`, `reference_panel_dir`, `parse_pvar`, `parse_psam`, `read_pgen_genotypes`, `match_scoring_to_pvar`, `aggregate_distributions`, `distribution_quality_issues`, `reference_distribution_audit_issues`, `enrich_distributions`, `ancestry_percentile`, `ReferenceDistribution`, `ScoringOutcome`, `BatchScoringResult`, `REFERENCE_PANELS`, `DEFAULT_PANEL`, `REFERENCE_FASTA`, `download_reference_fasta`, `reference_fasta_path`, `resolve_reference_alleles`, `Chip`, `RefSource`, `RestorationScope`, `ReferenceUniverse`, `prepare_reference_universe`, `__version__`, `__package_name__`.
...
Cleaned metadata parquets (including `publications.parquet`, `score_development_ancestry.parquet`, and `trait_prevalence.parquet`) are synced to/from the HuggingFace dataset repo `just-dna-seq/pgs-catalog` under the `data/metadata/` prefix. The metadata files synced by `push_pgs_catalog`/`pull_cleaned_parquets` are enumerated in `hf.CLEANED_PARQUET_FILES`. The HF token is resolved from: explicit argument > `.env` file (via `python-dotenv`) > `HF_TOKEN` environment variable. CLI commands: `just-prs catalog bulk clean-metadata`, `push-catalog`, `pull-hf`.
...
- **Server-side grid selection MUST be re-projected onto the loaded rows after every sort/filter/scroll, keyed by the grid's own stable column (CRITICAL).** The MUI selection model is keyed by the positional `__row_id__` that `lazyframe_grid()` renumbers on every sort/filter, so it is *not* a durable selection store — keep the durable selection in your own var (`selected_pgs_ids`, `selected_traits`, `metadata_selected_ids`) and re-derive the MUI model from it whenever the loaded rows change. Use the shared pure helpers `prs_ui.mixin.loaded_grid_selection_model(rows, selected_keys, key_field)` (rebuild the `{type:"include", ids:[...]}` model) and `merge_loaded_grid_selection(rows, current_selection, model, key_field)` (merge a checkbox event while preserving off-page/filtered-out selections). The `key_field` **differs per grid** and must match the column actually present in that grid's rows: `pgs_id` for the By-PRS scores grid, `trait` for the By-Trait grid, `"Polygenic Score (PGS) ID"` for the raw metadata grid. A concrete state must override `handle_lf_grid_row_selection`, `handle_lf_grid_sort`, `handle_lf_grid_filter`, `clear_lf_grid_filters`, and `handle_lf_grid_scroll_end` to call its own `_sync_loaded_*_selection()` after delegating to `LazyFrameGridMixin.<handler>(self, ...)`. **Re-syncing with the wrong key field (e.g. inheriting the `pgs_id`-keyed sync on a grid whose rows have no `pgs_id`) silently pushes an empty model on every sort — the durable badge stays correct while the checkboxes clear, and the next click collapses the selection.** Regression coverage: `prs-ui/tests/test_grid_selection.py`.
...
| `<cache>/reference_fasta/` | Ensembl primary-assembly `.fa` + `.fai` (precompute-only input for the reference-allele FASTA tier; `download_reference_fasta`) |
| `<cache>/liftover/` | UCSC chain files (`hg19ToHg38`/`hg38ToHg19.over.chain.gz`) auto-downloaded by `just_prs.liftover` |
| `<cache>/reference/` | Runtime reference-allele universe pulled from HF (build-aware `reference_allele_universe[_<build>].parquet` — GRCh38 unsuffixed; via `PRSCatalog._reference_universe_path(build)`) |
| `<cache>/percentiles/` | Distribution and quality parquets (`{panel}_distributions.parquet`, `{panel}_quality.parquet`, `{panel}_distribution_quality_issues.parquet`) plus the precompute output (build-aware `reference_allele_universe[_<build>].parquet`) |
...
| `check_dev_ancestry_valid` | `cleaned_pgs_metadata` | ERROR | `score_development_ancestry.parquet`: one row per PGS ID (no dupes), distribution fractions sum to ~1, `dev_is_multi_ancestry` agrees with `dev_n_ancestries`, sample sizes ≥ 0 |
...
| `check_reference_allele_universe_valid` | `reference_allele_universe` | ERROR | `ref_source ∈ {panel,fasta,unresolved}`, resolved rows non-null & unresolved rows null, FASTA REF is a single A/C/G/T, no duplicate `(chrom,pos)`, table non-empty |

Checks are included in job selections via `AssetSelection.checks_for_assets()` so they run automatically after their target asset materializes. Jobs that include checks: `full_pipeline`, `score_and_push`, `catalog_pipeline`, `chip_coverage_pipeline`, `reference_allele_pipeline`, `metadata_pipeline`.
...
|-------|-------|-------------|
| `illumina_gsa_manifest` | `external` (SourceAsset) | Illumina GSA v3 manifests (A2 = GRCh38, A1 = GRCh37), ~67 MB zip / ~648K markers each; GRCh38/A2 URL in metadata |
...
### Reference-allele universe pipeline assets

The `reference_allele_pipeline` job (`uv run pipeline reference-allele`, also part of `full_pipeline`) precomputes the reference base at every catalog scoring position lacking a `reference_allele`, so a plain WGS VCF recovers its ~50% genome-wide coverage at runtime (`variants_unscorable_absent → ~0`). The 3 GB genome FASTA is a **precompute-only input** — what ships to users is the small position-bounded parquet, not the genome. Position universe is bounded by the catalog's scoring files (built by iterating cached scoring parquets, the `chip_coverage` precedent).

| Asset | Group | What it does |
|-------|-------|-------------|
| `ensembl_grch38_fasta` | `external` (SourceAsset) | Ensembl GRCh38 primary-assembly FASTA (~900 MB gzip / ~3 GB uncompressed); URL in metadata |
| `reference_fasta` | `download` | `download_reference_fasta()` → `reference_fasta/*.fa` + `.fai` (decompresses plain-gzip to plain `.fa` for faidx; precompute-only) |
| `reference_allele_universe` | `compute` | Unions catalog scoring positions missing a `reference_allele` (`_ref_resolution_targets`), runs `resolve_reference_alleles()` (panel `.pvar` tier → FASTA SNV tier) → `percentiles/reference_allele_universe[_<build>].parquet` `(genome_build, chrom, pos, ref, ref_source)`. Build selected by `PRS_PIPELINE_GENOME_BUILD` (default GRCh38); metadata logs `genome_build`/`n_panel`/`n_fasta`/`n_unresolved`/`resolved_ratio` |
| `hf_reference_allele_universe` | `upload` | Pushes the build-aware `reference_allele_universe[_<build>].parquet` to `just-dna-seq/pgs-catalog` (`data/reference/`) via `push_reference_allele_universe(..., genome_build=)` |

### LD-proxy pipeline guardrails

`uv run pipeline ld-proxy` is memory-sensitive because it reads PLINK2 `.pgen` genotypes and builds LD correlations for untyped scoring positions. Never test it by launching the full job unless the user explicitly asks; use synthetic tests or tiny `--pgs-ids` / `--limit` pilots. The **catalog table is built once over the off-chip universe**, not per-PGS — a proxy for every off-chip catalog position computed exactly once (`build_ld_proxy_over_universe`). It reads the sample-agnostic `reference_allele_universe[_<build>].parquet` (the union of all catalog scoring positions, ~35M for GRCh38 — the `ref_missing` filter that built it is a no-op since scoring files carry no `reference_allele`), filters to off-chip (minus the chip's typed positions) via a **subprocess** DuckDB COPY (disk-spill, **interleaved by chromosome** so each batch spans all chromosomes and stays parallel) to `universe/_offchip_positions.parquet`, then processes that set in fixed **1M-position serial batches** (`DEFAULT_LD_UNIVERSE_BATCH_SIZE`), each a resumable `universe/batch_NNNNN.parquet`, and streaming-dedup-merges them (`merge_ld_proxy_tables`) into the single published table `{panel}_ld_proxy_{chip}_{build}.parquet`, pushed to HF via `push_ld_proxy_table`. This is **memory-deterministic** (peak RSS ~hundreds of MB — the off-chip set lives on disk and is sliced per batch, never resident) with a **clear linear ETA**, and eliminates the per-PGS redundancy (every shared target computed once instead of once per referencing score). **The DuckDB materialize MUST run in a subprocess** (`_materialize_offchip_universe_subprocess`): its glibc-retained virtual memory must be reclaimed before the parent opens the concurrent `pgenlib` readers under `RLIMIT_AS`, or opening them trips `MemoryError` in `PgenReader.__cinit__`. **The old global union staging path — a catalog-wide union held in *Python memory* at build time — must still not be reintroduced**; the universe set lives on disk and is processed in bounded slices, which satisfies that rule. LD proxies are **position-keyed** (the LD table schema has no `pgs_id` column), so the dedup-merge is lossless (a target shared by N scores has a byte-identical, position-deterministic proxy row) and `compute_array_prs` pulls the one table **once** via `pull_ld_proxy_table`, reusing it across every scored PGS (`apply_ld_proxies` joins on target position, never `pgs_id`). A **per-PGS path is retained for targeted/interactive single-score builds**: `--pgs-ids` → `build_ld_proxy_batch` → per-PGS intermediates `percentiles/ld_proxy/{panel}/{chip}/{build}/{pgs_id}.parquet` → merge (with a `PRS_LD_MAX_UNTYPED_TARGETS` cap, default 2M, that records over-cap scores as `skipped`); the per-PGS HF push/pull path was removed in favor of the single table. `--full-catalog` / `PRS_LD_FULL_CATALOG=1` runs the universe build over all off-chip positions (publishes to HF); `--limit N` bounds it to the first N off-chip positions (pilot, no HF push). Run the full build under OS containment (for example `systemd-run --user --scope -p MemoryMax=30G env PRS_DUCKDB_MEMORY_LIMIT=12GB uv run pipeline ld-proxy --panel 1000g --full-catalog --headless --memory-limit-gb 24 --workers 8`); `PRS_DUCKDB_MEMORY_LIMIT` caps the subprocess materialize. LD-specific gap/retry sensors are gated behind `PRS_LD_ENABLE_SENSORS=1` or `PRS_LD_FULL_CATALOG=1` so they never silently launch heavy LD work. Guardrails are controlled by: `PRS_LD_MEMORY_LIMIT_GB` (explicit cooperative RSS cap), `PRS_LD_MEMORY_LIMIT_PERCENT` (default 65% of RAM), `PRS_LD_TARGET_BATCH_SIZE` (default 10000 untyped targets submitted per batch), `PRS_LD_MAX_TARGETS_PER_CHUNK` (default 256 in-memory targets per correlation chunk), `PRS_LD_CHUNK_SIZE_BP` (default 250000 bp), `PRS_LD_MAX_WORKERS` (default 1), `PRS_LD_DUCKDB_MEMORY_LIMIT`, and `PRS_LD_BLAS_THREADS` (default 1). Lower `--limit`, `PRS_LD_TARGET_BATCH_SIZE`, `PRS_LD_MAX_TARGETS_PER_CHUNK`, and `PRS_LD_CHUNK_SIZE_BP` before raising workers. The pipeline should stream temporary parquet parts and keep only `PRS_LD_MAX_WORKERS` target batches in flight for the current PGS ID, never all catalog positions in Python memory.
...
- Reference batch scoring quality reports must have non-null `variants_total`, `variants_matched`, and `match_rate` for current per-PGS score parquets. `compute_reference_prs_polars()` persists these fields into each `reference_scores/{panel}/{pgs_id}/scores.parquet`, and `_aggregate_single_pgs()` must carry them into `ScoringOutcome`; otherwise `reference_distribution_audit_issues()` correctly emits `quality_match_metadata_missing` even after a no-cache run.
- To repair missing reference quality match metadata without recomputing pgen PRS scores, use `uv run prs reference backfill-quality --panel 1000g`. It reuses cached `reference_scores/{panel}/{pgs_id}/scores.parquet`, recomputes only scoring-file-to-pvar match counts, rewrites `{panel}_quality.parquet` and audit issues, and persists match metadata back into per-PGS score parquets. Partial trial runs must use `--output-subdir` so they do not overwrite full percentiles files.
